...
#new codes
  ...
  while rclpy.ok():
    ...
    try:
      ...
      pose2.y = 1.0
      pose2.z = 0.0
      orient2 = generateOrientation()
      #position = generatePosition()

      #orientation = generateOrientation()
      
      point1 = sendGoal(pose1, orient1)
      
      #goal_handle = sendGoal(position, orientation)

      status = checkResult(point1)
      #wait for goal to be reached

      #set the next goal
      point2 = sendGoal(pose2, orient2)
      status = checkResult(point2)
      #wait for goal to be reached
